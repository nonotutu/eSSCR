...
class ServicesController < InheritedResources::Base
  ...
  def update
    ...
    # corrige les heures des servolos : avance ceux du début
      servolos = Servolo.where("service_id = ? AND starts_at < ?", @service.id, new_depart_at || new_surplace_at || new_starts_at)
      servolos.each do |servolo|
        servolo.starts_at = new_depart_at || new_surplace_at || new_starts_at
        servolo.save
      end
